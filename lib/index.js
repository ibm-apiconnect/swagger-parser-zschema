    ...
    try {
      ...
      if (schema.swagger) {
        ...
        if (schema.paths === undefined) {
          ...
          // ULTRA-LENIENT: Normalize version string to handle cases like "3.0" without patch version
          let normalizedVersion = schema.openapi;
          if (normalizedVersion && !normalizedVersion.match(/\.\d+\.\d+$/)) {
            // Version is like "3.0" or "3.1", add ".0" patch version
            normalizedVersion = normalizedVersion + ".0";
          }
          
          if (supportedVersions.indexOf(normalizedVersion) === -1) {
            // Check if it's at least a 3.x version
            if (!normalizedVersion || !normalizedVersion.startsWith('3.')) {
              throw new SyntaxError(
                `Unsupported OpenAPI version: ${schema.openapi}. ` +
                  `Swagger Parser only supports versions ${supportedVersions.join(", ")}`,
              );
            }
            //  Accept any 3.x version, normalize to closest supported version
            if (normalizedVersion.startsWith('3.1')) {
              schema.openapi = '3.1.0';
            } else if (normalizedVersion.startsWith('3.0')) {
              schema.openapi = '3.0.0';
            } else {
              // Default to 3.0.0 for any other 3.x
              schema.openapi = '3.0.0';
            }
          } else if (normalizedVersion !== schema.openapi) {
            // Update the schema with normalized version
            schema.openapi = normalizedVersion;
          }
        }
